...
def create_nerf(args):
    ...
    # Create optimizer
    train_params = [{'params': model_fine.rgb_linear.parameters()}, {'params': model.rgb_linear.parameters()}]
    if args.use_alpha:
        train_params.append({'params': model_fine.alpha_linear.parameters()})
        train_params.append({'params': model.alpha_linear.parameters()})
    if args.use_feature:
        train_params.append({'params': model_fine.feature_linear.parameters()})
        train_params.append({'params': model.feature_linear.parameters()})
    if args.use_view:
        train_params.append({'params': model_fine.views_linears.parameters()})
        train_params.append({'params': model.views_linears.parameters()})

    #optimizer = torch.optim.Adam(params=grad_vars, lr=args.lrate, betas=(0.9, 0.999)) # original NeRF optimizer
    optimizer = torch.optim.Adam(train_params, lr=args.lrate, betas=(0.9, 0.999))
